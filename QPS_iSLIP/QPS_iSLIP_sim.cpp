...
int main(int argc, char* argv[]) {
  ...
  std::string fn = "../results/QPS_iSLIP-" + std::to_string(sw.params.N) + "-" +
  ...
  QPS_iSLIP qps_islip_s(sw.params.N, seed1,
                        sw.params.accept_buffer); /* create a iLQF scheduler */

  RV rv_for_tgen(sw.params.seed);

  main_run(sw, rv_for_tgen, &qps_islip_s, results);
